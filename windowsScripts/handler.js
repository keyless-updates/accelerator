...
routes['/keyless.io.learn'] = function(browserReq, browserRes){
	...
	postHelper(browserReq, browserRes, function () {	
		...
		if(siteURL && learningData){
			
			var inputObject = JSON.stringify({ siteURL:siteURL, learningData:learningData });
			var options = {		        
...
routes['/keyless.io.getOneCredential'] = function(browserReq, browserRes){
	postHelper(browserReq, browserRes, function () {	
		var body = browserReq.body;
		var siteURL = ''; var credentialType = ''; var accountName = ''; 
 		for(key in body){
			var data = JSON.parse(key);	
			siteURL = data.siteURL;	
...
function handleRequest(browserReq, browserRes){
    ...
    if(targetUrl.path in staticFiles){
        ...
        function createServerReq (){
            ...
            for(var i=0; i< global.fakePasswords; i++){
                if(bodyAsString.indexOf(global.fakePasswords[i].value > -1)){
...
//                            console.log('global.keylessCookie '+global.keylessCookie);
...
function prepareRequestToKeylessPipe(browserReq, url){
    ...
    return { url: "https://keyless.io/accelerator.pipe", jar: false, method: browserReq.method, encoding:null, headers: browserReq.headers, followAllRedirects: false, followRedirect:false };
}
